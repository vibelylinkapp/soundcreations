...
/**
 * Prototype-style page hero (scoped .sc-proto): photo background with eyebrow, title, lead and two CTAs.
 * Returns first-party HTML. Render INSIDE a .sc-proto wrapper.
 *
 * @param array $args Overrides: eyebrow, title, desc, img, cta1_label, cta1_url, cta2_label, cta2_url, dot.
 * @return string
 */
function sc_proto_pagehero( $args = array() ) {
	$d = wp_parse_args(
		$args,
		array(
			'eyebrow'    => 'Consult - Design - Distribute - Integrate - Support',
			'title'      => '',
			'desc'       => '',
			'img'        => SC_THEME_URI . '/assets/img/hero-poster.jpg',
			'cta1_label' => 'Request a Consultation',
			'cta1_url'   => home_url( '/request-a-consultation/' ),
			'cta2_label' => 'Explore Our Solutions',
			'cta2_url'   => home_url( '/solutions/' ),
			'dot'        => true,
		)
	);
	$dot = $d['dot'] ? '<span class="dot">.</span>' : '';
	ob_start();
	?>
	<section class="hero">
		<div class="photo" style="background-image:url('<?php echo esc_url( $d['img'] ); ?>')"></div>
		<div class="container">
			<div>
				<div class="eyebrow"><?php echo esc_html( $d['eyebrow'] ); ?></div>
				<h1><?php echo esc_html( $d['title'] ); echo $dot; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static markup. ?></h1>
				<?php if ( strlen( (string) $d['desc'] ) ) : ?><p><?php echo esc_html( $d['desc'] ); ?></p><?php endif; ?>
				<div class="actions">
					<?php if ( strlen( (string) $d['cta1_label'] ) ) : ?><a class="btn primary" href="<?php echo esc_url( $d['cta1_url'] ); ?>"><?php echo esc_html( $d['cta1_label'] ); ?> &rarr;</a><?php endif; ?>
					<?php if ( strlen( (string) $d['cta2_label'] ) ) : ?><a class="btn secondary" href="<?php echo esc_url( $d['cta2_url'] ); ?>"><?php echo esc_html( $d['cta2_label'] ); ?> &rarr;</a><?php endif; ?>
				</div>
			</div>
		</div>
	</section>
...
/**
 * Prototype-style red CTA box (scoped .sc-proto).
 *
 * @param array $args Overrides: eyebrow, title, text, img, label, url, pad_top_zero.
 * @return string
 */
function sc_proto_cta( $args = array() ) {
	$d = wp_parse_args(
		$args,
		array(
			'eyebrow'      => 'Ready when you are',
			'title'        => 'Have a project in mind?',
			'text'         => 'Let us design and deliver the right solution for your space and application.',
			'img'          => SC_THEME_URI . '/assets/img/cta-building.jpg',
			'label'        => 'Request a Consultation',
			'url'          => home_url( '/request-a-consultation/' ),
			'pad_top_zero' => true,
		)
	);
	$pad = $d['pad_top_zero'] ? ' style="padding-top:0"' : '';
	ob_start();
	?>
	<section class="section"<?php echo $pad; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static attribute. ?>>
		<div class="container">
			<div class="ctabox">
				<div class="bg" style="background:linear-gradient(90deg,rgba(111,4,20,.96),rgba(111,4,20,.55)),url('<?php echo esc_url( $d['img'] ); ?>') center/cover"></div>
				<div class="content">
					<div class="eyebrow"><?php echo esc_html( $d['eyebrow'] ); ?></div>
					<h2><?php echo esc_html( $d['title'] ); ?></h2>
					<p><?php echo esc_html( $d['text'] ); ?></p>
					<a class="btn primary" href="<?php echo esc_url( $d['url'] ); ?>"><?php echo esc_html( $d['label'] ); ?> &rarr;</a>
				</div>
			</div>
		</div>
	</section>
